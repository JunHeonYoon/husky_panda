...
"""
This version acquires [q, min_dist] dataset in determined distribution by coll_ratio.
"""
...
def main(args):
    ...
    coll_ratio = [r / sum(args.coll_ratio) for r in args.coll_ratio] # normalize
    ...
    def work(id, result):
        ...
        num_desired = [int(args.num_q / args.num_th * r) for r in coll_ratio]
        num_collected = [0, 0, 0]
        while sum(num_collected) < sum(num_desired):
            q = np.random.uniform(low=joint_limit[0], high=joint_limit[1], size=7)
            pc.display(q)
            min_dist = pc.min_distance(q)*100 # [m] -> [cm]

            if min_dist <= 1:
                if num_collected[0] < num_desired[0]:
                    q_set.append(q)
                    min_dist_set.append(min_dist)
                    num_collected[0] += 1 
            elif (min_dist > 1) & (min_dist <= 5):
                if num_collected[1] < num_desired[1]:
                    q_set.append(q)
                    min_dist_set.append(min_dist)
                    num_collected[1] += 1 
            else:
                if num_collected[2] < num_desired[2]:
                    q_set.append(q)
                    min_dist_set.append(min_dist)
                    num_collected[2] += 1 

            if (sum(num_collected) / sum(num_desired)*100) % 10 == 0 :
                t1 = time.time()
                print("{0:.1f}% of dataset accomplished on th:{1}! (Time: {2:.02f})".format(sum(num_collected) / sum(num_desired)*100, id, t1-t0))
                print([c/d*100 for c, d in zip(num_collected, num_desired)])
    ...
    print("Total number of data: {} (coll: {}, close:{}, free: {})".format(dataset["min_dist"].size, 
                                                                           np.sum(dataset["min_dist"]<=1), 
                                                                           np.sum((dataset["min_dist"]>1) & (dataset["min_dist"]<=5)), 
    ...
    with open(data_dir + "/param_setting.txt", "w", encoding='UTF-8') as f:
        params = {"number of dataset": dataset["min_dist"].size,
                  "number of collsion data": np.sum(dataset["min_dist"]<=1),
                  "number of close data": np.sum((dataset["min_dist"]>1) & (dataset["min_dist"]<=5)),
...
if __name__ == '__main__':
    ...
    parser.add_argument("--coll_ratio", type=float, nargs=3, default=[0.5, 0.35, 0.15], help="0: collide / 1: close (1cm < and <= 5cm) / 2: far ( 5cm < )")
