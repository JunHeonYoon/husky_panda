...
class CollisionNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(np.array([self.dataset[env_idx[i]]["q"][q_idx[i]] for i in range(len(idx))]), dtype=torch.float32), \
               torch.tensor(np.array([self.dataset[i]["occupancy"] for i in env_idx]), dtype=torch.float32), \
...
def main(args):
    ...
    log_dir = 'log/env_collsion_ver1/' + data_dir
    chkpt_dir = 'model/checkpoints/env_collsion_ver1/' + data_dir
    model_dir = 'model/env_collsion_ver1/' + data_dir
    ...
    for q, occ, min_dist in test_data_loader:
        test_q, test_occ, test_min_dist = q.to(device, dtype=torch.float32).squeeze(), occ.to(device, dtype=torch.float32), min_dist.to(device, dtype=torch.float32).squeeze()

    for epoch in range(args.epochs):
        loader_tqdm = tqdm.tqdm(train_data_loader)

        # for training
        for q, occ, min_dist in loader_tqdm:
            train_q, train_occ, train_min_dist = q.to(device, dtype=torch.float32).squeeze(), occ.to(device, dtype=torch.float32), min_dist.to(device, dtype=torch.float32).squeeze()

            collnet.train()
            with torch.cuda.amp.autocast():
                train_min_dist_pred = collnet.forward(train_q, train_occ).squeeze()
        ...
        collnet.eval()
        with torch.cuda.amp.autocast():
            test_min_dist_pred = collnet.forward(test_q, test_occ).squeeze()
